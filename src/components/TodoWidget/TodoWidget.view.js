        ...
        <div className="d-block mx-auto">
            ...
            <div className="todo__header">
                ...
                <span className="todo__filter">
                    <Button size="sm" onClick={props.setVisibilityFilter(VisibilityFilters.SHOW_ALL)}>All</Button>
                    <Button size="sm" onClick={props.setVisibilityFilter(VisibilityFilters.SHOW_ACTIVE)}>Active</Button>
                    <Button size="sm" onClick={props.setVisibilityFilter(VisibilityFilters.SHOW_COMPLETED)}>Completed</Button>
                </span>
            </div>
            <Table>
                <tbody>
                    {   
                        props.todos.map(todo => (
                            <tr key={todo.id}>
                                <th onClick={props.updateTodo(todo.id)} className="task__toggle">{todo.isCompleted ? <FaCheckCircleO size={26} /> : <FaCircleO size={26} /> }</th>
                                <td className={todo.isCompleted ? "task--completed" : ""}>{todo.task}</td>
                                <td onClick={props.deleteTodo(todo.id)} className="task__remove"><FaClose size={24}/></td>
                            </tr>
                        ))
                    }
                </tbody>
