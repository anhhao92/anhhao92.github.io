...
export const getColumns = (state, props) => {
    ...
    props.configs.selectedColumns.forEach(col => {
        displayColumns.push({
            dataField: col,
            text: capitalizeFirstLetter(col)
        })
    })
    return displayColumns
}

export const getSelectedColumns = (state, props) => props.configs.selectedColumns;
...
export const getAvailableColumns = createSelector (
    ...
    (data, selectedColumns) => data && data.length ? Object.keys(data[0]).filter(s => !selectedColumns.includes(s)) : []
